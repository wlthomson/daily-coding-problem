...
void freq_sort(string str) {
    ...
    for(string::const_iterator cit_str = str.cbegin(); cit_str != str.cend(); ++cit_str) {
        it_d = d.find(*cit_str);
        if (it_d == d.end()) {
            d[*cit_str] = 1;
        } else {
    ...
    for(multiset<char_count>::const_iterator cit_set = freq_set.cbegin(); cit_set != freq_set.cend(); ++cit_set) {
