...
int main() {
  ...
  thread t1(execute, function, 1000);
  spawned_threads++;
  t1.detach();
  thread t2(execute, function, 3000);
  spawned_threads++;
  t2.detach();
  thread t3(execute, function, 2000);
  spawned_threads++;
  t3.detach();

  int started_threads = thread_counter_started.load();
  while (started_threads < spawned_threads) {
    started_threads = thread_counter_started.load();
  }
  cout << "All threads started" << endl;

  int finished_threads = thread_counter_finished.load();
  while (started_threads > finished_threads) {
